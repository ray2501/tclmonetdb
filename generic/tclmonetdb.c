...
static int MONET_STMT(void *cd, Tcl_Interp *interp, int objc,Tcl_Obj *const*objv){
  MonetDBStmt *pStmt;
  ...
  if (tsdPtr->initialized == 0) {
    ...
    tsdPtr->monetdb_hashtblPtr = (Tcl_HashTable *) ckalloc(sizeof(Tcl_HashTable));
  ...
  switch( (enum STMT_enum)choice ){
    ...
    case STMT_PARAM_STRING: {
      ...
      if (mapi_param_string(pStmt->hdl, field_number, sql_type, ptr, NULL) != MOK) {
    ...
    case STMT_PARAM_NUMERIC: {
      ...
      if( objc == 6){
         ...
         * Here I use char * pointer to record data, is is OK?
         */
        ...
        Tcl_WrongNumArgs(interp, 2, objv, "field_number scale precision numeric_string");
        return TCL_ERROR;
      }
       ...
       */
      if (mapi_param_numeric(pStmt->hdl, field_number, scale, precision, (void *) ptr) != MOK) {
          return_obj = Tcl_NewBooleanObj(0);
      } else {
          return_obj = Tcl_NewBooleanObj(1);
      }
    ...
    case STMT_FETCH_ROW_LIST: {
      ...
      if (result > 0) {
         ...
         if (field_count > 0) {
            for(field_number = 0; field_number < field_count; field_number++) {
                ...
                if(!result_str) {
                    ...
                    Tcl_ListObjAppendElement(interp, pResultStr, Tcl_NewStringObj(result_str, -1));
                }
            }
         }
      }
...
static int DbObjCmd(void *cd, Tcl_Interp *interp, int objc,Tcl_Obj *const*objv){
  ...
  switch( (enum DB_enum)choice ){
    ...
    case DB_MONET_VERSION: {
      ...
      */
      hdl = mapi_query(pDb->dbh, "select value from sys.environment where name = 'monet_version'");
      if(hdl == NULL) {
    ...
    case DB_QUERY: {
      ...
      if(pStmt->hdl == NULL) {
          ...
          newHashEntryPtr = Tcl_CreateHashEntry(tsdPtr->monetdb_hashtblPtr, handleName, &newvalue);
          Tcl_SetHashValue(newHashEntryPtr, pStmt);
          Tcl_MutexUnlock(&myMutex);

          Tcl_CreateObjCommand(interp, handleName, (Tcl_ObjCmdProc *) MONET_STMT,
...
static int MONETDB_MAIN(void *cd, Tcl_Interp *interp, int objc,Tcl_Obj *const*objv){
  MonetDBDATA *p;
  const char *zArg;
  int i;
  ...
  if( objc<2 || (objc&1)!=0 ){
    Tcl_WrongNumArgs(interp, 1, objv,
      "HANDLE ?-host HOST? ?-port PORT? ?-user username? ?-passwd password? ?-dbname DBNAME? "
    );
    return TCL_ERROR;
  }
